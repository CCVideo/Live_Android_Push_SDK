...
import com.bokecc.sdk.mobile.push.example.popup.SingleBtnPopup;
...
public class PushActivity extends BaseActivity<PushPresenter> implements PushContract.View {
    ...
    protected void setUpView(Bundle savedInstanceState) {
        ...
        if (mPushConfig != null && mPushConfig.orientation == PORTRAIT) {
        ...
        // 添加主持人模式监听
        mPresenter.addHostModeListener();
        // 添加异地登录被踢出监听
        mPresenter.addKickOutListener();
    }
...
//        mRoot.setLayoutParams(layoutParams);
    ...
    public void setMainSpeaker(boolean isMainSpeaker) {
        if (mainSpeakerPopup != null) {
            mainSpeakerPopup.dismiss(null);
        }
        String content = "您被授予主讲人权限，学员可看到您的视频画面";
        if (!isMainSpeaker) {
            content = "您的主讲权限被主持人收回";
        }
        mainSpeakerPopup = new SingleBtnPopup(this);
        mainSpeakerPopup.setOKText("确定");
        mainSpeakerPopup.setTip(content);
        mainSpeakerPopup.setOnOkClickListener(new SingleBtnPopup.OnOkClickListener() {
            @Override
            public void onOk() {
                mainSpeakerPopup.dismiss(null);
            }
        });
        mainSpeakerPopup.show(findViewById(android.R.id.content));
    ...
    public void endStream() {
        ...
        String content = "直播已停止";
        final SingleBtnPopup singleBtnPopup = new SingleBtnPopup(this);
        singleBtnPopup.setOKText("确定");
        singleBtnPopup.setTip(content);
        singleBtnPopup.setOnOkClickListener(new SingleBtnPopup.OnOkClickListener() {
            @Override
            public void onOk() {
    ...
    protected void onResume() {
        ...
        try {
            if (mPushSession != null) {
                int volume = mPushSession.getVolume();
                if (volume == 0) {
